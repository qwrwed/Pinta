...
- **Native Windows title bar** - uses the native Win32 title bar instead of GTK4/libadwaita client-side decorations:
  ...
  - The other modal dialogs (message prompts like "Save changes before closing?" and effect/adjustment dialogs) also make the main window inert while open - its native title-bar buttons are disabled and clicking them gives the system ding instead of starting to close the app under the dialog. (Unlike the native file dialogs above, these GTK-drawn dialogs don't flash, since Windows only flashes when its own native dialog has the focus.)
