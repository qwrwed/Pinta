...
- **Drag-and-drop: open or add as layer** - dropping an image file onto an open document prompts whether to open it as a new document or add it as a layer (Paint.NET behaviour), instead of always opening it. Choosing "Add Layer" inserts the dropped file above the existing layers, loaded through Pinta's own importers so layered `.ora`/`.pdn` files come in as their separate layers (preserving name, visibility, opacity, and blend mode) rather than a single flattened layer; the canvas grows (anchored top-left) if the dropped image is larger. Dropping onto an empty workspace opens the file as before. The same multi-layer import improvements apply to `Layers > Import from File...`, which previously produced a blank layer for `.ora`/`.pdn`.
...
- **Text tool improvements** - the text caret now blinks while editing; text can be moved by dragging its edge (with a move cursor shown on hover) as well as the existing right-drag; placed text shows its bounding box and can be re-edited with a plain click (Ctrl no longer required); clicking away keeps the text editable instead of baking it in (matching Escape); and the cursor reflects what a click will do (I-beam to place/edit, move arrows on the border, normal pointer where a click finishes editing). Relates to upstream issue [#633](https://github.com/PintaProject/Pinta/issues/633).
