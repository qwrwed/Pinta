...
- **`File > Open Recent` menu** - a hover-opening flyout listing recently opened/saved files (most recent first, up to 10), each with a thumbnail preview (rendered via Pinta's own loaders, so `.ora`/`.pdn` are shown too, not just standard image formats), plus a "Clear List" entry. The list is persisted between sessions, and entries that can no longer be opened are dropped automatically. Because GTK4 menus can't display images or host custom widgets inside a submenu, the flyout is a custom widget slotted into the menu, which requires Pinta to host its own menu bar.
