...
- **Native Windows title bar** - uses the native Win32 title bar instead of GTK4/libadwaita client-side decorations:
  ...
  - Keeps the main window in front when a modal dialog is dismissed: previously, dismissing a dialog (such as "Save changes before closing?") after switching to another app and back dropped the main window behind that other app.
